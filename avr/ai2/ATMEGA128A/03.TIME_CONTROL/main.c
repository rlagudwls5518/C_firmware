 ...
 * 03.TIME_CONTROL.c
 *
 * Created: 2026-06-15 오전 11:36:55
 * Author : kccistc
 */ 
...
250대의 펄스를 count(1ms) 하면 이곳으로 자동진입을 한다.
ISR은 가능한 짧게 작성한다. -> 왜 짧게 하느냐??  이벤트에 대한 상황처리만 하고 빠져나와야한다
*/
ISR(TIMER0_OVF_vect){
...
1.timer 0 을 초기화한다.
	...
	임베디드에서 가장 신경을 써야 할 부분이 초기화 하는 것이다.
	초기화가 잘못되면 이후가 다꼬인다.
2. 8bit가지고 1ms를 측정하는 timer/counter를 만들고자 한다.
  2-1 분주비 설정(64분주) 16000000Hz / 64 =>> 250,000Hz
  2-2 1주기가 잡아먹는 시간 계산 T = 1/f = 1/250,000 ->0.000004sec (4us) ==> 0.004ms
  2-3 8bit 가지고 count 하는 시간을 계산 ( 8bit timer 0V(Overflow))
 		0.004ms x 256개 ==> 0.001024sec --> (1.024ms) 
